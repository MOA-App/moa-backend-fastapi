...
class TestAddPermissionToRoleUseCase:
    ...
    async def test_raises_when_permission_not_found(
        self, usecase, mock_role_repository
    ):
        mock_role_repository.add_permission_to_role.side_effect = (
    ...
    async def test_raises_when_already_assigned(self, usecase, mock_role_repository):
        mock_role_repository.add_permission_to_role.side_effect = (
            RoleAlreadyAssignedException("Permissão já associada")
        )
...
class TestRemovePermissionFromRoleUseCase:
    ...
    async def test_raises_when_role_not_found(self, usecase, mock_role_repository):
        mock_role_repository.remove_permission_from_role.side_effect = (
    ...
    async def test_raises_when_permission_not_assigned(
        self, usecase, mock_role_repository
    ):
        mock_role_repository.remove_permission_from_role.side_effect = (
            RoleNotAssignedException("Permissão não está associada")
        )
