...
from app.modules.auth.application.dtos.permission.paginated_result import (
    PaginatedResult,
)
...
from app.modules.auth.application.dtos.permission.permission_queries import (
...
class ListPermissionsUseCase:
    ...
    async def execute(
        self, query: Optional[ListPermissionsQueryDTO] = None
        ...
        try:
            ...
            return PaginatedResult(
                items=[PermissionMapper.to_summary_dto(p) for p in page_items],
                total=total,
                page=query.page,
    ...
    async def list_by_resource(self, resource: str) -> List[PermissionSummaryDTO]:
        ...
        try:
            ...
            raise RepositoryException(
                operation=f"listar permissões do recurso {resource}", details=str(e)
            )
    ...
    async def group_by_resource(self) -> List[ResourceActionsDTO]:
        try:
            ...
            raise RepositoryException(
                operation="agrupar ações por recurso", details=str(e)
            )
