...
class CreatePermissionDTO(BaseModel):
    ...
    def validate_permission_format(cls, v: str) -> str:
        ...
        if len(parts) > 5:
            raise ValueError("Permissão não pode ter mais de 5 níveis de profundidade")
...
class UpdatePermissionDTO(BaseModel):
    ...
    descricao: Optional[str] = Field(
        None, max_length=500, description="Nova descrição da permissão"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"descricao": "Descrição atualizada da permissão"}
        }
    )
