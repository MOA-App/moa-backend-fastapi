...
class BulkCreatePermissionsDTO(BaseModel):
    ...
    permissions: List[CreatePermissionDTO] = Field(
        ..., min_length=1, max_length=50, description="Lista de permissões a criar"
    )
...
class BulkCreatePermissionsResponseDTO(BaseModel):
    ...
    skipped: List[str] = Field(
        default_factory=list, description="Permissões que já existiam (ignoradas)"
    )
    errors: List[dict] = Field(
        default_factory=list, description="Erros durante a criação"
    )
    total_created: int
    total_skipped: int
    total_errors: int
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "skipped": ["users.read"],
                "errors": [{"nome": "invalid.format", "error": "Formato inválido"}],
                "total_created": 3,
                "total_skipped": 1,
                "total_errors": 1,
            }
