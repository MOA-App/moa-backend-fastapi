...
class ListPermissionsQuery(BaseModel):
    ...
    search: Optional[str] = Field(
        default=None, max_length=100, description="Busca por nome ou descrição"
    )
    ...
    sort_by: Literal["nome", "data_criacao"] = Field(
        default="data_criacao", description="Campo para ordenação"
    )
    sort_order: Literal["asc", "desc"] = Field(
        default="desc", description="Ordem de ordenação"
    )
