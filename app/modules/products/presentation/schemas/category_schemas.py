...
class ErrorResponseSchema(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {"example": {"detail": "Categoria não encontrada"}}
    }
...
class ConflictErrorSchema(ErrorResponseSchema):
    ...
    model_config = {
        "json_schema_extra": {
            "example": {"detail": "Categoria com nome 'Eletrônicos' já existe"}
        }
    }
...
class CategoryListResponseSchema(BaseModel):
    ...
    items: List[dict] = Field(
        ..., description="Lista de categorias (CategoryResponseDTO)"
    )
