...
class ProductModel(Base, TimestampMixin):
    ...
    name = Column(String(255), nullable=False, index=True, comment="Nome do produto")
    description = Column(Text, nullable=True, comment="Descrição do produto")
    price = Column(Numeric(10, 2), nullable=False, comment="Preço do produto")
    sku = Column(
        String(50),
        unique=True,
        nullable=False,
    ...
    stock_quantity = Column(
        Integer, nullable=False, default=0, comment="Quantidade em estoque"
    )
    is_active = Column(
        Boolean, nullable=False, default=True, comment="Se o produto está ativo"
    )
